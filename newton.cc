...
void gsl_pca (const Nan::FunctionCallbackInfo<v8::Value>& info) {
    ...
    // Define the vectors Ident, _Ident, Media and Sigma, The last will store
    ...
    * Sigma=gsl_vector_calloc(n) ;
    ...
    // We read the variables from the matrix data, and we make the media = 0
    ...
    gsl_vector_set_all(ident,1);
    // the variables are prepared
    Dot(S,ident,result);
    // The varianze total is calculated
    ...
    gsl_matrix_free (M);gsl_matrix_free (V_T);gsl_matrix_free (_V_);gsl_vector_free (Ident);gsl_vector_free (_Ident);
